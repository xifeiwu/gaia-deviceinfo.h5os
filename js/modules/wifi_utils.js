 ...
 * @module WifiUtils
 */
define(['require','softkey_handler','shared/wifi_helper/wifi_helper'],function(require) {
  'use strict';
  ...
  var WifiUtils = {
    ...
    newListItem: function(options) {
      ...
      a.href = '#';
      a.dataset.wifiKey = this.getNetworkKey(network);
      a.appendChild(ssid);
      a.appendChild(small);
    ...
    initializeAuthFields: function(panel, network) {
      ...
      var eap = panel.querySelector('li.eap select');
      var certificate = panel.querySelector('li.server-certificate select');
      var submitButton = panel.querySelector('gaia-button[type=submit]');
      ...
      var checkPassword = function() {
        ...
        var isDisabled = !WifiHelper.isValidInput(key, password.value,
                            identity.value, eap.value) || isSSIDInvalid();
        submitButton.disabled = isDisabled;
        SoftkeyHandler.register(panel.id, {
          rsk: {
            name: (isDisabled) ? ' ' : 'ok',
            action: (isDisabled) ? function() {} : function() {
              submitButton.click();
            }
          }
        });
