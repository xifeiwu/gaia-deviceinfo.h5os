 ...
 *  };
 *
 *  WARNING: To use this library you need to include 'shared/l10n/l10n.js'
 *
 */
var DownloadUI = (function() {
  ...
  */
  ...
  function removeContainers() {
    window.dispatchEvent(new CustomEvent('downloadDialog-hide'));
    removeConfirm();
    removeActionMenu();
  }
  ...
  function createConfirm(type, req, downloads) {
    ...
    if (window.SoftKeysHelper) {
      SoftKeysHelper.registerKeys({
        lsk: (lButton) ?
          navigator.mozL10n.get(type.name + '_download_left_button') : '',
        dpe: (lButton) ?
          '' : navigator.mozL10n.get(type.name + '_download_right_button'),
        rsk: (lButton) ?
          navigator.mozL10n.get(type.name + '_download_right_button') : ''
      }, menu);
    }
    ...
    window.dispatchEvent(new CustomEvent('downloadDialog-show', {
      detail: {
        container: confirm
      }
    }));
  ...
  function doCreateActionMenu(req, fileName, actions) {
    ...
    if (window.SoftKeysHelper) {
      SoftKeysHelper.registerKeys({
        lsk: navigator.mozL10n.get('cancel'),
        rsk: ''
      }, menu);
    }
  ...
  function show(type, downloads, ignoreStyles) {
    ...
    window.setTimeout(function() {
      var libs = ['shared/download_lib/download_formatter.js'];
      if (!ignoreStyles) {
  ...
  function showActions(download) {
    ...
    window.setTimeout(function() {
      LazyLoader.load(['shared/mime_mapper/mime_mapper.js',
                       'shared/download_lib/download_formatter.js',
